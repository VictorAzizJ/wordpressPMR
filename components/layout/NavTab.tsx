...
/** Teal foot on every tab; matching teal underline on the current section, in the gaps between those blocks. */
export function NavTabIndicators({ active }: { active: boolean }) {
  return (
    <>
      <span
        className="pointer-events-none absolute inset-x-1.5 bottom-0 h-2 bg-pmr-teal"
        aria-hidden
      />
      {active ? (
        <span
          className="pointer-events-none absolute inset-x-0 bottom-0 z-[1] h-[3px] bg-pmr-teal"
          aria-hidden
        />
      ) : null}
    </>
